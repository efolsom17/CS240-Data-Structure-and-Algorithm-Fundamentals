...
The user is able to add new words to the dictionary with `add` and can view the dictionary with `dictionary`. The dictionary gets reset whenever you re-run the program. I could, in the future return to this and modify it such that when the user exits the program, we overwrite the `dictionary.txt` file that we read with an updated dictionary from the session. 
