...
Notice that in the above, when $\text{step#} \mod 3 \neq 1$, if we are moving the smallest disk, we move it to the left. If we are moving a larger disk, we move it to the right. 
...
Now a real pattern starts to emerge. The equivalency classes created by the step numbers corresponds to which rods we are moving disks between, while the even/oddness of the disk tells us what direction we are moving the disks between. 
