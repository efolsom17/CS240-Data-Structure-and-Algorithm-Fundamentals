...
- In step $2$, disk $2$ gets moved from start rod to auxillary rod (to the right).
...
Now a real pattern starts to emerge. The equivalency classes created by the step numbers corresponds to which rods we are moving disks between, while the even/oddness of the disk tells us what direction we are moving the disks between. The directions are switched depending on whether $n$ is even or odd.
...
```{plain}
going to need to modify my stackMove function to compare the values of things at the top of stacks
...
3. Implement a suggestion feature that suggests possible correct spellings for the misspelled words based on edit distance.
   ...
   For this I decided to create a list of pairs of the string and its corresponding boolean value returned from invoking the `misspelled()` function on the string. I then took all the values which according to `misspelled()` were misspelled and compared the edit distance to each string in the dictionary. I stored the key, edit distance pair in an array and then chose the 5 strings with the smallest edit distances. I computed the edit distance using levenshtein's distance algorithm. Initially I did it using the naive recursive method, but I adapted it to give it my best shot at dynamic programming as described in the following videos. 

4. Implement a Levenshtein distance algorithm to compute the edit distance between two words.
   
   To implement a Levenshtein distance algorithm to calculate the edit distance between two strings I first went to [Wikipedia](https://en.wikipedia.org/wiki/Levenshtein_distance). The piecewise representation of this algorithm made a lot of sense to me, so I decided to implement it in python as best I could directly from the piecewise function. After watching a [couple]() [of]() [YouTube]() videos related to spellcheckers, general python, and dynamic programing, I decided to give dynamic programming my best shot. ( actually scratch that I might go with the matrix method as shown on wikipedia.)
