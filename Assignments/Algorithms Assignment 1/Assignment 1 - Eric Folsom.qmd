...
I would hypothesize that the average time complexity for binary search is $\log_2(n)$, where $n$ is the number of items (indices) of an array.
...
ind_data = list(range(1999)) # indices of the data that we want, data=numbers.txt
...
print(operations_numbers)
```

Now we return back to R to visualize the results.
...
We find that the average number of operations our binary search algorithm took to be `r mean(results_sim)` operations. We can compare this to our hypothesized value of $11$.
...
function getXOR(arr1, arr2){ # Exclusive Or, in A or B, but not both.
  ...
  # the above for loop takes n1*n2 iterations to complete
